...
router.post('/pic/update', upload.single('profile_url'), async (req, res) => {
    db = req.app.locals.db;
    var destination = path.join(__dirname, '/uploads') + req.file.filename;
    ...
    function starter() {
        //checking file type
        fs.stat(path.join(__dirname, '/uploads') + req.file.filename, async function (err, stats) {
...
router.get('/:username', async (req, res, next) => {
    ...
    try {
        if (result && Array.isArray(result.petAdded) && result.petAdded.length > 0) {
            petAdded = await db.collection('petsinfo').find({ _id: { $in: result.petAdded } }).toArray();
    ...
    try {
        if (result && Array.isArray(result.petLiked) && result.petLiked.length > 0) {
            petLiked = await db.collection('petsinfo').find({ _id: { $in: result.petLiked } }).toArray();
